...
static int rc_getsym(void)
{
  int atype, i = 1, warning = 0, c = rc_gchar();

  if( isdigit(c) ) atype = MDIGIT;
  else if( isalpha(c) ) atype = COMPSYM;
  else { rc_serror(5, NULL); return 0; }
  ...
  while( (c = rc_gchar() ) != '.') {
    ...
    else if( (isalpha (c) )&&(atype == COMPSYM) ) strings[i++] = c;
    else { rc_serror (5, NULL); return 0; }
  }
...
/* get an index of a variable terminated by any non-digit or non-letter,
 * and save it in str[]. */
static int rc_getind(void)
{
  int i = 1, warning = 0, c = rc_gchar();

  if( !isalnum(c) ) { rc_serror(3, NULL); return 0; }
...
static int rc_getnumb(int d)
{
  int c = rc_gchar(), warning = 0;
  unsigned long i = d - '0', k = (unsigned long)MAX_UNSIGNED_INT;

  while( isdigit(c) ) {
    // The next few lines are strange because otherwise the "C" compiler
    // produces an integer overflow warning
    if(warning||( i > ( (k - c - '0')/10 + (k - c - '0')%10 ) ) ) warning = 1;
    else i = i*10 + c - '0';
    c = rc_gchar();
  }
...
static int rc_getstr(int quote)
{
  int c = rc_gchar();
  ...
  if(c == ESCAPE) {
    ...
    if(c == '\'' || c == '"' || c == '(' || c == ')' || c == '<' || c == '>' || c == ESCAPE) strings[i++] = c;
    else if(c == 'n') strings[i++] = '\n';
    else if(c == 'r') strings[i++] = '\r';
    else if(c == 't') strings[i++] = '\t';
    else if(c == 'x') {
      int i_w;
      char c_w = 0;

      for(i_w = 0; i_w < 2; i_w++) {
        if( EOF == ( c = rc_gchar() ) ) { rc_serror(6, NULL); return 0; }
        if( isxdigit(c) ) {
          c = (c <= '9') ? c - '0' : ( (isupper (c) ) ? c - 'A' + 10 : c - 'a' + 10);
          // Nemytykh 21.06.2001      c_w += ((i)? c: 16 * c);
          c_w += i_w ? c : 16*c;
        }
        else { rc_serror(6, NULL); return 0; }
      }
      strings[i++] = c_w;
    }
    else rc_serror(6, NULL);
  } // end of if ...ESCAPE...
  else strings[i++] = c;
...
int rc_gettoken(void)
{
  ...
  token = 0;
  int c = (globsav == 0) ? rc_gchar() : globsav;
  globsav = 0;
  while(token == 0) {
    ...
    switch(c) {
      ...
      case '/':
        ...
        rc_serror (1,NULL);
          ...
          do {
            while(c != '*') {
              if(c == EOF) { rc_serror(2, NULL); return EOF; }
              c = rc_gchar();
            }
          } while( (c = rc_gchar() ) != '/' );
          c = rc_gchar();
      ...
      case 'e': case 's': case 't': /*case 'w' :*/
        ...
        if(d == '.') {// variable.
          ...
          else { token = rc_getid1(c); globsav = d; }
          // Nemytykh .... ]
        str[0] = c;
        }
        else rc_serror(20, NULL);
        c = ' ';
        break;
      case ESCAPE:
        c = rc_gchar();
        if(c == 'x') {
          ...
          char c_w = 0;
          for(i_w = 0; i_w < 2; i_w++) {
            if( EOF == ( c = rc_gchar () ) ) rc_serror(7, NULL);
            if( isxdigit(c) ) {
              c = (c <= '9') ? c - '0': ( isupper(c) ?  c - 'A' + 10 : c - 'a' + 10 );
              c_w += (i_w) ? c : 16*c;
            }
            else rc_serror(7, NULL);
          }
          v = c_w;
          token = ASCIIV;
          c = ' ';
      ...
      case '<': // left concretization bracket
        ...
        token = LCBRAK;
        switch(c) {
          case '+': strcpy(str, bi[ADD].fname); break;
          case '-': strcpy(str, bi[SUB].fname); break;
          case '*': strcpy(str, bi[MUL].fname); break;
          case '/': strcpy(str, bi[DIV].fname); break;
          case '%': strcpy(str, bi[MOD].fname); break;
          case '?': strcpy(str, "Residue"); break;
          case '.': c = rc_gchar();
          default:
...
int rc_serror(int code, char *str) {
  ...
  switch(code) {
    ...
    case 4:
      ...
      sprintf(ca_err + strlen(ca_err),
              " 4. Illegal character: %c (%d)", *str, *str);
      break;
    case 5:
      // Was. Shura. 27.05.99
    ...
    case 201:
      ...
      sprintf(ca_err + strlen(ca_err),
              "201. Doubly defined function %s . May be the function is built.", str);
      break;
    case 202:
      // Was. Shura. 27.05.99
    ...
    case 205:
      // Nemytykh 09.08.2002
      sprintf(ca_err + strlen(ca_err), "205. Number greater than 2^%ld-1=%lu ",
              sizeof(int)*8, MAX_UNSIGNED_INT);
      break;
    default:
      // Was. Shura. 27.05.99
  ...
  //else fprintf (fdlis, "\n");
  if(line_no > 0) sprintf(ca_err + strlen(ca_err), " on line %5d.", line_no);
  fprintf(fdlis, "%s\n", ca_err);
...
/* print warning  */
int rc_swarn(int code)
{
  fprintf(fdlis, "warning: ");
  switch(code) {
    case 1: fprintf(fdlis, " 1. Identifier too long (excess ignored) "); break;
    case 2: fprintf(fdlis, " 2. Index too long (excess ignored) "); break;
    case 3: fprintf(fdlis, " 3. ASCII value over 256 "); break;
    case 4: fprintf(fdlis, " 4. Empty string (ignored) "); break;
    case 5:
      // Nemytykh 08.08.2002
      //fprintf (fdlis," 5. Number greater than %ld (shorten)",MAXINT);
      fprintf(fdlis," 5. Number greater than 2^%ld-1=%lu (shorten)",
              sizeof(int)*8, MAX_UNSIGNED_INT);
      break;
    case 6: fprintf(fdlis, " 6. The same index for 2 diff variables "); break;
