...
int rc_gchar(void) {
  if(cbuf[sc] == '\n') {
    ...
    putc('\n', fdlis);
    if(sc >= MAXSTR) { rc_serror(11,NULL); sc = MAXSTR; }
...
static void print_rasl_inst(struct rasl_instruction *r)
{
  int rasl_ins = r->code;
  ...
  switch(rasl_ins) {
    ...
    case LBL:
      printf("LBL: #%d\n", r->p.i); break;
    case ECOND:
      printf("   %s LABEL#%d\n", rasl_code(rasl_ins), r->p.i); break;
    case SETB:// takes 2 integer arguments.
      printf("   %s %d %d\n", rasl_code(rasl_ins), r->p.d.i1, r->p.d.i2); break;
    ...
    case SYM: case SYMR: case NS: case LENS:
      printf("   %s %c\n", rasl_code(rasl_ins), r->p.c); break;
